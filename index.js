...
app.post('/api/create-checkout-session', async (req, res) => {
  try {
    ...
    // This allows us to charge in the user's local currency
    const sessionConfig = {
      ...
      line_items: [
        {
          price_data: {
            ...
            product: productId,
            unit_amount: (amount || 149) * 100, // Stripe uses cents
            recurring: {
              interval: 'month',
            },
          },
...
app.post('/api/create-portal-session', async (req, res) => {
  try {
    ...
    if (!customerId) {
      // If no customer ID, try to find by email or create portal link page
